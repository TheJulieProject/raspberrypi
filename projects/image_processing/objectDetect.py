...
cascadePaths = (
                '/home/pi/raspberrypi/projects/image_processing/',             
                ) 
...
class ObjectDetector(object):
    ...
    def __init__(self,cascadeName="face"):
        self.storage = CreateMemStorage(0)
        ...
        self.cascade = Load(self.cascade_name)
        if( self.cascade ):
            if verbose:
                print "cascade loaded..."
        else:
            raise exceptions.IOError("Could not locate cascade file.")
        self.cvWin = NamedWindow( "result", 1 ) # is this needed? return value?
    ...
    def detectObject(self,img):
        ...
        ClearMemStorage( self.storage )
    
        
        if( self.cascade ):
            t = GetTickCount()
            objects = HaarDetectObjects( small_img, self.cascade, self.storage,
