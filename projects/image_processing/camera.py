...
class Camera():
    ...
    def __init__(self, device, size, mode, imageType='opencv'):
        ...
        self.capture = CreateCameraCapture(self.device)

        # set the wanted image size from the camera
        SetCaptureProperty (self.capture, CV_CAP_PROP_FRAME_WIDTH, self.width)
        SetCaptureProperty (self.capture, CV_CAP_PROP_FRAME_HEIGHT, self.height)
    ...
    def get_image(self,surface=None,*argv,**argd):
        ...
        if self.imageType == "pygame":
            try:
                surfarray.blit_array(surface,conversionUtils.cv2SurfArray(QueryFrame(self.capture)))
                return surface
            except:
                return surfarray.make_surface(conversionUtils.cv2SurfArray(QueryFrame(self.capture))).convert()
    ...
    def stop(self):
        return ReleaseCapture(self.capture)
...
def list_cameras():
    ...
    for i in range(3):
        try:
            capture = CreateCameraCapture( i )  # Must be a better way of doing this...
...
def opencvSnap(dev,size):
    ...
    cvWin = NamedWindow( "Opencv Rendering and Capture", 0 )
